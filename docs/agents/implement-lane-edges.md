...
| 2. Nothing changed? | `worktreeChanges()` — `git status --porcelain` on exactly the answered paths. Empty → escalate, comment `nothingToBuildNote()`, outcome `nothing-to-build`. An implementer that leaves the checkout as trunk has it is a real outcome, not a bug — but the run changed nothing the recompute reads, so a ticket left open here is rung again on the next pass, and forever: the strike ladder counts failures, and this run succeeded. `needs-human` is what ends it ([ADR-0192](../adr/0192-a-run-that-produces-no-pull-request-holds-its-ticket-because.md)) |
...
| 5. The fails rule, judged on the pushed diff | Commits the answer, rebases onto trunk when this lane opts in (`git fetch origin main && git rebase origin/main`; conflict → abort, release the claim, escalate, comment `rebaseConflictNote()`, outcome `rebase-conflict`, resolved by a human, never guessed at), and pushes the claimed branch with `--no-verify` (node 04b already ran the hook's gate on this tree) — before any judge runs, so a run that dies right after can never lose a paid answer. `judgeFailsEdits(git diff, declaredPaths)` then judges that pushed diff, refusing if a removed `test.fails(`/`it.fails(` line's paired addition is anything other than that same line with `.fails` dropped — unless that file is one of rung two's own `declaredEdits` paths, which is exempt. Refused → `dispatchMechanicWanted()` for the ticket instead of `escalateToOwner()`, comment `failsRuleNote()`, outcome `fails-rule-refused`. No `needs-human`: the mechanic gets the whole tree, including the gate, and this ticket's own `failsRuleNote()` comment as its brief, and the claim is left standing since the branch already carries the pushed answer — a false positive here is a bug in the gate, not in the run |
| 6. Open the PR | `gh pr create` then `dispatchVerify()` — the exact door lane 06 answers, [documented there](verify-lane-edges.md#node-00-the-two-doors-stop). A non-empty `declaredEdits` adds a `## Edits outside the ticket's fence` section to the PR body and the same text as a ticket comment (`declaredEditsNote()`), so the review lane sees it before judging the diff. If node 04b's gate was still red, `needs-human` and `gateRedNote()` land on the ticket right after |
...
| land the answer (node 06) | — | reads the worktree; re-writes and removes the answered paths | commits, rebases, pushes (`--no-verify`), opens the PR | comments; `needs-human` on an immutable-set refusal, a rebase conflict or a red gate; `dispatchMechanicWanted()` and no `needs-human` on a fails-rule refusal; posts `declaredEditsNote()` when rung two declared any |
...
| after push, before the PR opens | node 06 step 5 | The fails rule — a `test.fails(` test was edited beyond turning it on, and the edit is not one of rung two's own declared edits. This refusal keeps the pushed answer and dispatches the mechanic instead of stopping here |
