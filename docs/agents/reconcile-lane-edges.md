...
| **`needs-human` on a ticket** | is the owner's hold: the recompute never dispatches such a ticket, whatever its strikes say. Which is why a dead Acceptance, Implement or Mechanic job's own `if: always()` step clears the lane label and stops there rather than stamping `needs-human` as the other lanes' does ([venues.md](venues.md)): a lane that hands its ticket to the owner on its first death nails it down before rung one, and the ladder above never runs. #539 was the one that showed it |
