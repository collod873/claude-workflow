...
A fake restates its author's belief, written in the same hour from the same misreading, so it cannot disagree with him. Lane 04's delivery check read a `state` GitHub does not serve, could never return true, and cost a whole PRD while its unit tests stayed green.
