...
Lane 04 fires on every `issues.closed` event but judges only a close whose `state_reason` is `completed`. A *not planned* or *duplicate* close asserts no work was delivered, so there is nothing for a record to be about; refusing one would overrule a decision, not verify a claim.
