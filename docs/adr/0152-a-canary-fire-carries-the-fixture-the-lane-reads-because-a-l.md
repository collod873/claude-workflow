...
door: Spec's `sheet-accepted` dispatch woke the lane at the right ref, then died on `ISSUE_NUMBER
must be a positive integer; got ""`. An issue-driven lane reads what an earlier lane left on an
issue, not the event, and Spec's collector refuses anything but a decision sheet and an accept
payload (ADR-0058).

So a lane needing an issue declares its fixture in `shared/canary-fixture.ts`, which `bin/canary`
seeds and passes in the payload. A lane with no fixture fires as before. The fixture is written
