...
A change to part of a ruling is an edit to that ADR. A successor declares `supersedes:` only when the ruling reverses whole (ADR-0045), because the one machine that reads the edge retires what it names: the #571 audit found eleven live rulings retired by notes that only amended them.
