...
describe("one Stop hook owns turn-end in this checkout", () => {
  it("#367.1/#558.2: no Stop entry anywhere in this checkout, and the turn gate runs from the roster", () => {
    expect(settings).not.toHaveProperty("hooks");
    expect(roster.Stop).not.toContain("gauntlet.sh");
    expect(roster.PostToolUse).toContain("gauntlet.sh");
  });
