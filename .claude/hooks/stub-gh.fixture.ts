...
/**
 * @fixture Reached only from the suites, by design: a stub `gh` on a caller's path would be a
 * caller that never talked to the tracker.
 */
...
export function stubGh(payload: IssuePayload | string): GhStub {
  ...
  const comments = (issue.comments ?? []).map(({ body, createdAt }) => ({ body, createdAt }));
  const answer = JSON.stringify(JSON.stringify({ body: issue.body, comments }));
  writeFileSync(
    path,
