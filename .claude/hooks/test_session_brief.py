...
STUB_GH.write_text(
    ...
    "    label = flag('--label')\n"
    "    if label:\n"
    "        print(json.dumps(table.get(label, [])))\n"
    "    else:\n"
    "        seen = {}\n"
    "        for name, issues in table.items():\n"
    "            for issue in issues:\n"
    "                labels = [l if isinstance(l, dict) else {'name': l} for l in issue.get('labels') or []]\n"
    "                if name not in [l.get('name') for l in labels]:\n"
    "                    labels.append({'name': name})\n"
    "                seen[issue['number']] = dict(issue, labels=labels)\n"
