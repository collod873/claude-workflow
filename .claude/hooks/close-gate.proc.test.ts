 ...
 *
 * The `gh` every case points at through `AGENT_SKILLS_GH` (`bin/gh_support.py`'s override)
 * is `shared/stub-gh.fixture.ts`'s, answering the one call this hook makes — `gh issue view
 * --json body,comments` — from a fixed payload, so no case reaches the network or the tracker.
 */
...
/** A close of #55 with no closing record, against a tracker holding `A_TICKET`. */
