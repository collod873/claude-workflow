...
Before slicing, sweep the spec for the seams the batch will share. The disjoint-files rule prevents a collision of path; it does nothing about a collision of shape: N tickets that all need the same helper touch no file in common, so each hand-rolls its own copy instead. Catching that seam before slicing, rather than discovering it afterward as a rework batch, is what this step is for.

Split the spec into slices (its own sections, or a sensible chunking if it has none) and dispatch one **seam-sweep subagent** per slice, by stub, in the foreground (ADR-0168). Each subagent's brief: read its assigned slice, then search this codebase for a seam that already covers what the slice needs, before proposing anything new: reuse always outranks building. Combine every subagent's return into the **seam manifest**.
...
Dispatch a fresh-context **audit agent** on the list above (the Agent tool, by stub, in the foreground, ADR-0168) after the breakdown is computed and before publishing. It grades the breakdown against the same four sizing calls a maintainer used to be asked:
...
Every concern the audit raises is written as a **flag** in its report, decided this run on the agent's own recommendation, never a block on publishing, and never a question relayed back to a human. Where the recommendation is to merge, split, or re-edge tickets, apply that edit to the list before publishing; report every flag, applied or not. No path through this skill waits for a human's approval of the breakdown.
...
- **GitHub** → do NOT publish issue-by-issue. Write the whole approved breakdown as one graph JSON file (in the scratchpad), shaped `{ "parent": <prd number>, "plan": <Plan> }`, where `<Plan>` is the same array-of-slices shape step 4/5 already produced (`dependsOn` by position in that array, not by a sibling key), then run `~/bin/publish-issue-graph <graph.json>` in a single command. It hands the file straight to lane 03's `sliceAndPublish` (`.Workflow/agent-workflows/to-tickets/publish-issue-graph.cli.ts`), which is where inward-only edges, rooted claimed paths and criteria shape are enforced — a refusal there names the offending slice's position and the rule before any issue is created, so nothing here restates those rules as prose to remember. On success it prints one row per published slice (position, number, title); paste that table in your report.
