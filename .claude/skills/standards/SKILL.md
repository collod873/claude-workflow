...
not `/drain`, not a hook, not a schedule. Run it when you want the standards of
...
every write happens inside the dispatched step, never here.

## The chain

Each of the first three steps below is dispatched as a **fresh-context subagent**: the Agent tool, by
stub, in the foreground (ADR-0168): a one-line prompt naming the step's `SKILL.md` (e.g. "Read
...
4. **`/drain <spec>`**: only when step 3 ran, and **inline, not as a subagent**: read
   ...
   is a foreman, and a foreman runs inline, never as a subagent. From here `/drain`
...
give, never a consequence of this run. Step 1 computes its own scope from the last
...
- Every `[flag: …]` verdict from step 2, with what was decided: the maintainer's after-the-fact
  review inbox; reversal is a hand edit of the line plus a revert or follow-up ticket.
