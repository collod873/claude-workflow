...
~/.agents/skills/bin/link-deps "<scratchpad>/wt-<ticket>" "<repo>"
```

Name the base tip explicitly as above: the harness's own worktree isolation forks from the repo's default branch, which is rarely the tip a map is working from. Then hand the tree's path in the dispatch and say in the same breath that the main checkout is **read-only** to that agent: a source to read context from, never a target for a write, an install, a commit, or a `git` command that moves `HEAD`. The mechanics live once, in `~/.agents/skills/docs/agents/agent-worktrees.md`: what `link-deps` provisions and why, which gitignored files do not follow into a worktree, and what a read-only path is still good for.
