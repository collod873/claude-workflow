...
- **Run rows**: every hook's fires are in `~/.claude/logs/<hook>-YYYY-MM-DD.jsonl`; `~/bin/hook-report` reads across them. One row, three writers, one per language a hook is written in: `_hook.run_row()` + `_hook.append_log()` in Python, `runRow()` + `appendLog()` from `_hook.mjs` in JavaScript, `hook_run_row <verdict> [key=value ...]` from `_hook.sh` in bash (a shim that hands the payload to `_hook.mjs`, so JSON is parsed in one place). All three are this repo's own files under `.claude/hooks/` and `.claude/hooks/lib/`, reached through `~/.claude/hooks/` symlinks into the machine's one checkout, so there is no second copy that can drift. A hook in another language uses its language's writer, never a private append: a private copy is where `tool_use_id` went missing from two repos' rows before the merge, and `bin/hook-report` reads only the shared shape.
