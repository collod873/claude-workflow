...
what it is all *for*. When a proposal arrives (a skill, a hook, a connector, an eighth era) it gets
scored against §2. A proposal that fails a constraint is not a smaller version of the goal; it is a
different goal.
...
dies by ~month 3 regardless of quality.* Reinforced by F2, where the system becomes its own biggest
...
3. **No mechanism points backwards.** Every ADR, every lint rule, every `CODING_STANDARDS.md` entry:
   **not one has been asked whether it caught anything.** That file has exactly one exit
...
| **W4** | Decisions live next to the code they govern; the endpoint is documentation a test suite can fail on |
