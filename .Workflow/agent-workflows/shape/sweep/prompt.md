...
its GitHub issues. Nothing else — no web.
...
- Issues, **open and closed**: `gh issue list --state all --search '<terms>' --json number,title,state,url`.
  Skip `#{{ISSUE_NUMBER}}` — that is the idea you are sweeping for, and it matches its own title.
...
- a `url`, which is what the owner taps. `gh issue list --json …,url` gives you an issue's; a
  ...
  `docs/adr/`.
- a `bearing` — why it bears on *this* idea, **in one line**. The sheet funds three prior-art lines
