...
function machineTicketBody(repository: string, run: RunSummary, machineSha: string, path: string, logTail: string): string {
  ...
  return [
    ...
    `failed with its failing step naming \`${path}\`, a path the machine checkout tracks`,
    "(ADR-0141). That makes it this repository's own defect, filed here",
...
function machineImmutableTicketBody(repository: string, run: RunSummary, machineSha: string, path: string, logTail: string): string {
  return [
    ...
    "it (ADR-0054), so no implementer could ever build a ticket claiming it, so it is filed `by-hand`",
