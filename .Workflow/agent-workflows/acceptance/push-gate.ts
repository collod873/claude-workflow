...
import { runVitestJson, type TestFailure, type TestRunResult } from "../shared/vitest-json";
...
// The runner and its report shape live in `shared/vitest-json.ts` — `implement/implement.ts`
// reads the same classification to find a ticket's failing tests — and are re-exported here so
// this gate's callers keep one door.
export { runVitestJson, type TestRunResult };
