...
import { PUSH_ATTEMPTS, PUSH_BACKOFF_SECONDS, pushToTrunk } from "../shared/push-to-trunk";
...
interface PushRace extends Harness {
  pushes: () => number;
}

function losingPushes(losses: number): PushRace {
  let pushes = 0;
  const raced = harness({
    git: (args) => {
      if (args[0] === "push" && ++pushes <= losses) throw new Error("rejected: non-fast-forward");
      return "";
    },
  });
  return { ...raced, pushes: () => pushes };
}
...
describe("#519: the land job's own steps are the lane's error path", () => {
  ...
  it("retries a lost push race, and lands when one of the retries wins", async () => {
    const { deps, slept } = losingPushes(2);
    expect(await landAuthoredBatch(deps, request())).toEqual({ outcome: "landed" });
...
test("#541.3: land reaches trunk through the shared module rather than a second copy of the loop", async () => {
  ...
  const viaLand = losingPushes(Number.POSITIVE_INFINITY);
  expect((await landAuthoredBatch(viaLand.deps, request())).outcome).toBe("needs-human");
  expect(viaLand.pushes()).toBe(direct.pushes());
...
test("#541.4: land reports its own sentence when the push retries are exhausted, not the helper's", async () => {
  ...
  expect(helperWhy).not.toBe("");

  const outcome = await landAuthoredBatch(refusing((args) => args[0] === "push").deps, request());
  expect(outcome.outcome).toBe("needs-human");
  const why = "why" in outcome ? outcome.why : "";
  expect(why).not.toBe(helperWhy);
...
test("#541.5: the whole check contract passes, land landing through the shared retry on the shared constants", async () => {
  ...
  const viaLand = losingPushes(2);
  expect(await landAuthoredBatch(viaLand.deps, request())).toEqual({ outcome: "landed" });
  expect(viaLand.pushes()).toBe(3);
  expect(viaLand.slept).toEqual([PUSH_BACKOFF_SECONDS, 2 * PUSH_BACKOFF_SECONDS]);
});
