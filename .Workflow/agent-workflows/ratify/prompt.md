...
- **Author the rule** as an inline rule object in `eslint.config.js`, the way
  ...
  There is no `eslint-rules/` directory and no plugin package — one file.
...
- **Give it a real id.** `landedAs` is the rule id exactly as it appears in the
  `rules` map — `namespace/rule-name`. It is the key everything downstream
...
### 2. Prose entry — not mechanisable
