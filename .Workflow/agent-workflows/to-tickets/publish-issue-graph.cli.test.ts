...
function preseededTracker(firstIssueNumber: number, createdIssues: CreateIssueInput[]) {
  const issueIds: Record<number, number> = {};
  for (let i = 1; i <= 20; i++) {
    const number = firstIssueNumber + i;
    issueIds[number] = number * 1000 + 7;
  }
  return trackerMemory({ firstIssueNumber, issueIds, createdIssues });
}
...
describe("publish-issue-graph.cli", () => {
  ...
  test("#603.3: an out-of-range dependsOn exits nonzero, naming the slice position and the rule, with zero issues created", () => {
    const createdIssues: CreateIssueInput[] = [];
    const tracker = preseededTracker(99, createdIssues);
    const plan = [slice({ title: "Root" }), slice({ title: "Off the end", dependsOn: [9] })];
    const file = writeGraph(42, plan);

    expect(() => runPublishIssueGraphCli([file], tracker)).toThrow(/slice 2 \("Off the end"\).*out-of-range/);
    expect(createdIssues).toHaveLength(0);
  });

  test("#603.3: a dependsOn cycle exits nonzero, naming the slice positions and the rule, with zero issues created", () => {
    const createdIssues: CreateIssueInput[] = [];
    const tracker = preseededTracker(99, createdIssues);
    const plan = [
      slice({ title: "Root" }),
    ...
    expect(() => runPublishIssueGraphCli([file], tracker)).toThrow(/dependency cycle detected/);
    expect(createdIssues).toHaveLength(0);
  });

  test("#603.3: a criterion with no check: marker exits nonzero, naming the slice position and the rule, with zero issues created", () => {
    const createdIssues: CreateIssueInput[] = [];
    const tracker = preseededTracker(99, createdIssues);
    const plan = [slice({ title: "No marker", acceptanceCriteria: ["Just prose, no marker at all"] })];
    const file = writeGraph(42, plan);

    expect(() => runPublishIssueGraphCli([file], tracker)).toThrow(/slice 1 \("No marker"\).*check/);
    expect(createdIssues).toHaveLength(0);
  });
  ...
  test("#603.4: two slices with no edge between them whose filesClaimed overlap are published, with one stderr line naming both positions and the overlapping path", () => {
    ...
    runPublishIssueGraphCli([file], tracker);
