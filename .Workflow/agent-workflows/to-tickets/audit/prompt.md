...
6. Keep every slice's prose inside its ceilings, including slices you merge or re-word, since the tool refuses a field that runs over: `whatToBuild` at most 400 characters, `whyNotMerged` at most 200, and each `acceptanceCriteria` entry at most 200. A merge that would push `whatToBuild` past its ceiling is evidence the separation was earned. There is no ceiling on how many files a slice claims or how many criteria it carries.
