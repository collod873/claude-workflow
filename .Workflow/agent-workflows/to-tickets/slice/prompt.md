...
## The ticket contract you publish to

Every ticket this lane publishes takes the shape below, which is `docs/agents/ticket-format.md`'s
spec-sub-issue variant, inlined here rather than restated, so this prompt and the doc can never
drift apart. There is no other ticket-shape document to go and read.
...
5. Validate every acceptance criterion against the headless-checkability gate, and write each one in the shape the ticket contract above defines its `check:` marker. The publisher refuses the whole plan before it files anything, so a slice carrying one unreadable or unanswerable criterion costs the run.
...
7. Root every path you name, per the `## Files claimed` rule in the ticket contract above. Two independent runs read this ticket and neither can ask the other or see the other's work (the acceptance author turns it into tests, the implementer turns it into code) so a path you leave relative is a decision you have handed to both of them, and they are not obliged to answer it the same way. When that happens the implementation goes red against tests written to a different reading, and the only signal anyone downstream gets is a failing test that looks exactly like bad code. The publisher refuses the whole plan for an unrooted path. A path is rooted when its first segment is a top-level entry of the repository (`.Workflow`, `docs`, `bin`, `.github`, …); `shape/accept.ts` and `shared/gh.ts` are not rooted, `.Workflow/agent-workflows/shape/accept.ts` is. The one abbreviation allowed is a path **this same slice** already carries in full in its own `filesClaimed` — a path claimed by a different slice does not count, and neither does a bare suffix like `.test.ts`. When in doubt spell the full path: it is never refused. Name each path on its own and in full, never several joined into one token — `.Workflow/agent-workflows/shared/tracker.ts/tracker-gh.ts` reads as a single path that does not exist, so write the two out separately even when they sit in the same directory.
...
10. Keep the prose inside its ceilings, since the tool refuses a field that runs over, so aim under them rather than at them: `whatToBuild` at most 400 characters, `whyNotMerged` at most 200, and each `acceptanceCriteria` entry at most 200. There is no ceiling on how many criteria a slice carries. `filesClaimed` has a hard one too, set by `claimLimit` in `.Workflow/agent-workflows/shared/ticket-shape.rules.json`, and the publisher refuses the whole plan over it. Claim every file the slice modifies — then, if that exceeds `claimLimit`, the slice is too big and the answer is to cut it, never to under-claim. Lane 04's acceptance author inlines the full contents of every claimed file into one prompt and has a single wall-clock budget for authoring, the gate and one repair round; a nine-file slice spent seventeen minutes on its first pass alone and died three times with nothing authored (#539). `claimLimit` is what fits, not a taste, and there is no slice the publisher lets past it.
