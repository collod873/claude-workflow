...
function gateBullets(record: string): string[] {
  const { stdout } = python(
    `${loadAsModule("close_gate", CLOSE_GATE, join(REPO_ROOT, ".claude/hooks"))}\nprint(json.dumps(module.CLOSING_BULLET_RE.findall(sys.stdin.read())))`,
    record,
  );
...
function writerReading(): WriterReading {
  return JSON.parse(runPython(WRITER, [ROOT, RULES])) as WriterReading;
}

function admitted(records: Array<{ text: string; criteria: number | null }>): GateVerdict[] {
  return JSON.parse(runPython(GATE, [HOOKS, JSON.stringify({ records })])) as GateVerdict[];
}
