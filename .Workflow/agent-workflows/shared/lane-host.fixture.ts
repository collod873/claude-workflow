...
export function githubHostingLanes(options: LaneHostOptions = {}): LaneHost {
  const fallback = createFakeGh();
  const calls: string[][] = [];

  const answerLane = (args: string[]): string | undefined => {
  ...
  const gh: GhExec = (args) => {
    calls.push([...args]);
    return options.answer?.(args) ?? answerLane(args) ?? fallback.gh(args);
  };
  return { gh, calls, dispatches: fallback.dispatches };
}
