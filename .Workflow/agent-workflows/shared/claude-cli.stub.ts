...
/**
 * A stub `claude` binary, placed first on `PATH`, that delivers `response`
 * as the model's answer — for a test that exercises a stage's real CLI end
 * to end (argv, extraction, schema, handoff write, exit code) without
 * spawning one.
 *
...
export function stubClaudeCli(
  ...
  const stream =
    [
      ...
      JSON.stringify({ type: "result", subtype: "success", result: response, num_turns: 1 }),
    ].join("\n") + "\n";
