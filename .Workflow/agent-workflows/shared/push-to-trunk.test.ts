...
test.fails("#541.1: a push rejected on its first attempt succeeds on a later one", async () => {
  const seam = pushSeam(1);
  await expect(pushToTrunk(seam.deps)).resolves.toBeUndefined();
  expect(seam.pushes()).toBe(2);
});

test.fails("#541.1: exhausting every push attempt raises rather than reporting success", async () => {
...
test.fails("#541.2: PUSH_ATTEMPTS and PUSH_BACKOFF_SECONDS have one home, the shared module, and govern its retry", async () => {
