...
describe("regenerateArtifacts", () => {
   ...
   * to, which is the failure that used to take an implement run down with it.
   */
  it("regenerates and returns only the artifacts already present, never the ones an enrolled repository never seeded", () => {
    ...
    const root = rootWith([present!.path]);
    const { exec, calls } = recordingExec();

    const result = regenerateArtifacts(exec, root, () => {});

    expect(calls).toEqual([{ generator: present!.generator, root }]);
    expect(result).toEqual([present!.path]);
   ...
   * the path present so the caller still `git add`s whatever the generator left behind.
   */
  it("logs a failing generator rather than throwing, and still reports the artifact present", () => {
    const root = rootWith([GENERATED_ARTIFACTS[0]!.path]);
    const logged: string[] = [];
    ...
    expect(result).toEqual([GENERATED_ARTIFACTS[0]!.path]);
