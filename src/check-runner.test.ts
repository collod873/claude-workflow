...
describe("src/check-runner.ts passes a check only when it ran and measured something (#662)", () => {
  ...
  it("names each criterion whose check already passes and leaves the red ones alone", () => {
    const green = "npx vitest run --config vitest.config.ts done";
    const red = "npx vitest run --config vitest.config.ts todo";
