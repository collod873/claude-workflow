...
export function linesAllowed(part: Part): number {
  return part.file.startsWith("src/hooks/") ? 1 : Math.min(part.lines ?? 1, MOST_LINES);
}
...
export function post(posting: Posting, gh: Gh, registry: Part[] = parts): { refusals: string[]; said: string } {
  ...
  if (kind === undefined) return { refusals: [`${posting.kind} is not a kind src/post.ts writes: ${Object.keys(KINDS).join(", ")}`], said: "" };
