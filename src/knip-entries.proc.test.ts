...
function planted(registry: string[]): string {
  ...
  copyFileSync(join(REPO, "knip.config.ts"), join(root, "knip.config.ts"));
  writeFileSync(join(root, "src", "parts.ts"), `export const parts = [\n${registry.map((file) => `  { name: "${file}", file: "${file}", stops: "" },\n`).join("")}];\n`);
  for (const name of PLANTED) {
    script(join(root, "bin", name), `node src/${name}.ts\n`);
    writeFileSync(join(root, "src", `${name}.ts`), `export const ${name} = 1;\n`);
    writeFileSync(join(root, "src", `${name}.test.ts`), `import { ${name} } from "./${name}.ts";\nconsole.log(${name});\n`);
  }
  return root;
}
...
describe("knip counts its entries from the registered parts, not from the tests (#710)", () => {
  it("fails an export that only a test imports, and passes one a registered part reaches", () => {
    const untouched = unused(planted(["knip.config.ts", "bin/reached"]));

    expect(untouched.stdout).toContain("src/lonely.ts");
    expect(untouched.stdout).not.toContain("src/reached.ts");
    expect(untouched.status).toBe(1);

    const called = unused(planted(["knip.config.ts", "bin/reached", "bin/lonely"]));
