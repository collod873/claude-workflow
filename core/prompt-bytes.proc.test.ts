...
describe("every prompt holds a byte ceiling that only ever shrinks (#663)", () => {
  ...
  it("refuses a prompt grown past its ceiling and records the smaller number when one shrinks", () => {
    ...
    copyFileSync(join(REPO, "core", "prompt-bytes.ts"), copy);
  ...
  it("names a stage that hires a model whose prompt nothing measures", () => {
    ...
    writeFileSync(join(copy, "core", "reviewer.ts"), 'spawnSync("claude", argv);\n');
    writeFileSync(join(copy, "core", "quiet.ts"), 'spawnSync("gh", argv);\n');
    expect(unmeasured(copy, PROMPTS)).toEqual(["core/reviewer.ts hires a model and no prompt of that name is measured"]);
  });

  it("passes from the repo and says one line", () => {
    const { status, stdout, stderr } = execute("node", REPO, {}, ["core/prompt-bytes.ts"]);
