...
const TYPE_ERROR = "core/a.ts(3,5): error TS2322: Type 'string' is not assignable to type 'number'.\n".repeat(40).trim();
...
describe("core/check says one line and keeps each failing tool's full output in a log it names (#683)", () => {
  it("names only the failed tools, where each one points, and a log holding each one's full output", () => {
    ...
    expect(line).toMatch(/^core\/check: FAILED typecheck core\/a\.ts:3, test a\.test\.ts:4; log /);
  ...
  it("drops the locations before the line would pass 200 characters, keeping every failed tool's name", () => {
    const deep = `core/${"a-rather-deep-folder/".repeat(6)}file.ts(12,1): error\n`;
    const { run } = checkRepo({ tsc: deep, eslint: deep, knip: deep, jscpd: deep, vitest: deep });

    const { line } = saidOneLine(run().stdout);

    expect(line.length).toBeLessThanOrEqual(200);
    expect(line).toMatch(/^core\/check: FAILED typecheck lint unused clones test; log /);
  });
