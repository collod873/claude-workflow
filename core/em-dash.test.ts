...
function filesUnder(tree: Tree, dir: string): string[] {
  return tree
    .entries(dir)
    ...
    .flatMap((entry) => {
      ...
      return tree.read(path) === undefined ? filesUnder(tree, path) : [path];
    });
}

function emDashesIn(tree: Tree): string[] {
  const inCore = filesUnder(tree, "core").flatMap((file) => emDashLines(tree.read(file) ?? "").map((line) => `${file}:${line}`));
...
describe("nothing in core/ and no doc a session loads carries an em dash (#681)", () => {
  ...
  it("holds today's core/ and loaded docs at none", () => {
    expect(emDashesIn(onDisk(REPO))).toEqual([]);
  });

  it("names the file and line of a planted em dash in core/ or a loaded doc, and leaves what no session loads alone", () => {
