...
describe("the stable check acts as the App, and fails red when its key is missing (#712)", () => {
  ...
  it("mints the token before core/check runs, and never falls back to GITHUB_TOKEN", () => {
    const steps = workflowSteps(CHECK);
    const minted = steps.findIndex((step) => step.run?.includes("core/bin/app-token"));
    const checked = steps.findIndex((step) => step.run?.trim() === "core/check");
