...
const DASH = "\u2014";
const LINE = 200;
const TEST_CHECK = "npx vitest run --config core/vitest.config.ts ticket-shape";
const GREP_CHECK = "grep -q ticketRefusals core/post.ts";
...
const misshapen: [string, string, unknown[]][] = [
  ...
  [
    ...
    body({ claimed: ["- core/ticket-shape.ts", "- `core/**/*.test.ts`"] }),
...
describe("core/ticket-shape refuses a ticket body that hides what was meant (#662)", () => {
    ...
    expect(
      ticketRefusals(
        body({
          ...
          claimed: ["- core/ticket-shape.ts", "- `core/post.ts`"],
        }),
      ),
    ).toEqual([]);
  });
