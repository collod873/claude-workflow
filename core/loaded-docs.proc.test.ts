...
describe("the docs every session loads never grow in total, and name no path to nothing (#663)", () => {
  ...
  it("names the doc and line of a planted path to nothing, and leaves placeholders, urls, other repos and unbuilt enforcers alone", () => {
    const repo = committedDocs();
    plant(repo, "core/check", "");
    plant(repo, "CLAUDE.md", [
      "# Planted",
      "",
      "Run `core/check`, never `core/gone.ts`; see [the charter](docs/agents/charter.md) and [the lanes](docs/agents/lanes.md).",
      "Branches are `ticket/<n>`, globs `core/**/*.ts`, commands `/ratify`, and [the tracker](https://github.com/collod873/claude-workflow#top).",
      "`anthropics/claude-code-action` is another repo; `bin/` is gone.",
      "",
    ].join("\n"));
    plant(repo, "docs/agents/layers/one-ticket.md", "# One ticket\n\n[Charter](../charter.md), [ADR](../../adr/0200-gone.md)\n");
    plant(repo, "docs/agents/charter.md", "| Rule | Enforcer |\n|---|---|\n| Nothing names `core/unbuilt.md` | `core/unbuilt.test.ts` |\n");
    plant(repo, ".claude/skills/tdd/SKILL.md", skill("tdd", "Test first, as `docs/nowhere.md` says.", "The body names `core/body-only.ts`."));

    expect(pathsToNothing(repo, loadedDocs(onDisk(repo)))).toEqual([
      "CLAUDE.md:3 names core/gone.ts, which does not exist",
      "CLAUDE.md:3 names docs/agents/lanes.md, which does not exist",
      "CLAUDE.md:5 names bin/, which does not exist",
