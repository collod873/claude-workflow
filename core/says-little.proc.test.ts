...
function executableParts(): string[] {
  return [...new Set(parts.map((part) => part.file))].filter((file) => (statSync(join(REPO, file)).mode & 0o111) !== 0);
}
...
describe(`everything core/ prints fits in ${LIMIT} characters (#683)`, () => {
  it.each(executableParts())("%s stays under the limit on a passing and a failing run", (part) => {
